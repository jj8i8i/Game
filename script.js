...
// --- Initial Setup ---
const updateInputs = () => { /* ... same as before ... */ };
...
solveBtn.addEventListener('click', () => {
    ...
    solutionWrapper.classList.add('hidden'); // Hide results until finished
    ...
    // --- NEW: Handle messages (status or result) ---
    ...
    solverWorker.onerror = (e) => { /* ... same as before ... */ };
});
...
function displayResults(result) {
    ...
    if (currentSolutions.length > 0) {
        ...
        if (currentSolutions.length > 1) { /* ... same as before ... */ }
    } else {
...
function renderSolution(solution) { /* ... same as before ... */ }
function generateDerivationList(item) { /* ... same as before ... */ }
// Paste the full functions for updateInputs, displayResults, renderSolution, generateDerivationList from the previous answer.
// The code is identical, only the call in onmessage is changed. The full code is below for clarity.
...
modeSelect.addEventListener('change', updateInputs); updateInputs();
solveBtn.addEventListener('click', () => {
    const numbers = Array.from(inputsContainer.children).map(input => parseInt(input.value)).filter(n => !isNaN(n));
    const target = parseInt(targetInput.value);
    const level = levelSelect.value;
    const numInputs = parseInt(modeSelect.value);
    if (numbers.length !== numInputs || isNaN(target)) { alert('กรุณาป้อนข้อมูลให้ครบถ้วน'); return; }
    resultsContainer.classList.remove('hidden');
    statusContainer.classList.remove('hidden');
